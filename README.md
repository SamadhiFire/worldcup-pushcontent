...
- 默认每日目标生成 `MATCHDAY_DAILY_PUSH_TARGET=10` 条：先保留每场 1 条官方/赛况事实锚点，再用 X 热点里热度更高、话题更顺的角度补满。
- 默认单场最多生成 `MATCHDAY_MAX_PUSHES_PER_MATCH=10` 条，避免只有少数比赛时被旧的单场上限卡住。
- 因此每日写入飞书条数目标约为 10 条；当天完全没有比赛且没有可用热点时写入 0 条。
